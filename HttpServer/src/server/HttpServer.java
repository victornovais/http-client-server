	...
	public void runServer() {
		...
		try {
			while (true) {
				...
				try {
					index = TreatRequest.read("static/index.html");
					notFound = TreatRequest.read("static/404.html"); 
				} catch (IOException e) {
					e.printStackTrace();
				}
				...
				if (fullRequest != null && (fullRequest.startsWith("GET"))
					...
					if (requestPieces.get(1).equals("/") || requestPieces.get(1).equals("/index.html")) {
						...
						serverResponse.writeBytes("Request Method: " + requestPieces.get(0) + BREAK_LINE);
